...
<table class="rows">
    <thead>
        <tr>
            ...
            <th>Description</th>
            <th class="r" style="width: 90px;">Billed (BDT)</th>
            <th class="r" style="width: 90px;">Paid (BDT)</th>
            <th class="r" style="width: 100px;">Balance (BDT)</th>
        </tr>
    </thead>
    <tbody>
        @if ($from !== null || $statement['carryForward'] != 0.0)
            <tr class="carry">
        ...
        @forelse ($statement['rows'] as $row)
            <tr>
                ...
                <td class="r">{{ $row['debit'] > 0 ? $fmt($row['debit']) : '' }}</td>
                <td class="r">{{ $row['credit'] > 0 ? $fmt($row['credit']) : '' }}</td>
                <td class="r">{{ $fmt($row['balance']) }}</td>
            </tr>
        @empty
            <tr><td colspan="5" class="muted" style="font-style: italic;">No transactions in this period</td></tr>
        @endforelse
        <tr class="totals">
            <td colspan="2">Period totals</td>
            <td class="r">{{ $fmt($statement['totalDebit']) }}</td>
            <td class="r">{{ $fmt($statement['totalCredit']) }}</td>
            <td class="r"></td>
        </tr>
    </tbody>
</table>
...
<div class="closing" style="color: {{ $statement['closing'] > 0 ? '#C43D3D' : '#1E9E58' }};">
    Closing balance{{ $statement['closing'] > 0 ? ' (due to '.settings('company_name', 'us').')' : '' }}: {{ $fmt($statement['closing']) }} BDT
</div>

<div class="muted" style="margin-top: 24px; text-align: center; font-size: 8.5px;">
    Generated on {{ now()->format('d M Y H:i') }} · Cancelled and draft invoices are excluded.
</div>
